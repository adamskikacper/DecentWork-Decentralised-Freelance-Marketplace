...
export const getClientJobs = async (): Promise<JobsData> => {
 return await getClientJobsData();
};

export const getFreelancerJobs = async (): Promise<JobsData> => {
 return await getFreelancerJobsData();
};
...
const getClientJobsData = async (): Promise<JobsData> => {
 const allJobs = getClientJobsRaw();
...
const getFreelancerJobsData = async (): Promise<JobsData> => {
 const allJobs = getFreelancerJobsRaw();
 const activeJobs = allJobs.filter(
  (job) => job.status === "In Progress" || job.status === "Just Started"
 );
 const completedJobs = allJobs.filter((job) => job.status === "Completed");
...
const getFreelancerJobsRaw = (): Job[] => {
 return [
  {
   id: "contract1",
